...
local meta_name = unilib.core.name .. ":meta_light_"
...
-- (Used in unilib.wield_light.track_user_entity() below)
...
local function get_lighting_node(node_name)
...
local function level_of_lighting_node(node_name)
...
local function is_lightable_node(node_pos)
...
local function get_light_position(pos)
    ...
    if not cat_code_table[cat] then
        ...
        if cat_id >= 256 then
            ...
            unilib.utils.show_error(
                "../lib/effects/wield_light/wield_light_base.lua, get_light_category_id():" ..
                        " Wielded light category limit exceeded (maximum 256)"
            )
...
local function is_wieldlight_node(pos_vec)
    ...
    local name = string.sub(core.get_node(pos_vec).name, 1, #meta_name)
...
local function recalc_light(pos)
    ...
    max_light = math.min(max_light, unilib.constant.light_max)
...
local function lighting_node_of_level(light_level, component)
...
local function get_light_def(item_name)
    ...
    return math.min(
        ...
        unilib.constant.light_max
    ), itemdef.floodable

end

---------------------------------------------------------------------------------------------------
-- Shared functions (register API in original code)
---------------------------------------------------------------------------------------------------

function unilib.wield_light.register_lightable_node(full_name, override_table, custom_component)

    -- Was wielded_light.register_lightable_node()
    ...
    if type(full_name) ~= "string" then
        ...
        unilib.utils.show_error(
            string.format(
                "unilib.wield_light.register_lightable_node(): You must provide a node name to" ..
                        " be registered as lightable, type \"%s\" given",
                type(full_name)
            )
        )
    ...
    local orig_def_table = core.registered_nodes[full_name]
    if not orig_def_table then

        unilib.utils.show_error(
            string.format(
                "unilib.wield_light.register_lightable_node(): The node \"%s\" cannot be" ..
                        " registered as lightable because it does not exist",
                full_name
            )
        )
    ...
    if component_check_table[component] then
        ...
        unilib.utils.show_error(
            string.format(
                "unilib.wield_light.register_lightable_node(): The lighting component \"%s\"" ..
                        " cannot be used for \"%s\" as it is already used for \"%s\"",
                component,
                full_name,
    ...
    new_def_table.mod_origin = unilib.core.name
    ...
    for i = 1, unilib.constant.light_max do
...
function unilib.wield_light.register_shining_item(item_name, light_level, floodable)

    -- Was wielded_light.register_item_light()
...
function unilib.wield_light.register_item_floodable(item_name, floodable)

    -- Was wielded_light.register_item_floodable()
...
function unilib.wield_light.register_lightstep(callback)

    -- Was wielded_light.register_lightstep()
...
function unilib.wield_light.register_player_lightstep(callback)

    -- Was wielded_light.register_player_lightstep
...
---------------------------------------------------------------------------------------------------
-- Shared functions (support API in original code)
---------------------------------------------------------------------------------------------------

function unilib.wield_light.track_item_entity(obj, cat, item)

    -- Was wielded_light.track_item_entity()
...
function unilib.wield_light.track_user_entity(obj, cat, item)

    -- Was wielded_light.track_user_entity()
...
unilib.wield_light.register_player_lightstep(function (player)
    unilib.wield_light.track_user_entity(player, "wield", player:get_wielded_item():get_name())
end)

-- Register helper nodes (air, plus all water nodes)
unilib.wield_light.register_lightable_node("air", nil, "")
for part_name, data_table in pairs (unilib.global.generic_liquid_table) do

    if data_table.water_flag then
        unilib.wield_light.register_lightable_node(data_table.source_name, nil, part_name .. "_")
    end
