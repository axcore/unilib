...
---------------------------------------------------------------------------------------------------
-- calendar.lua
--      Set up calendar effects (original to unilib)
---------------------------------------------------------------------------------------------------
...
-- For standard calendars, 366, or 365 if unilib.calendar_leap_year_flag is false
...
-- The list is shifted to the left by unilib.calendar_week_start_offset, e.g. for the value 1,
--      it becomes {1, 2, 3, 4, 5, 6, 0} (thus the left-hand column is Monday)
...
local function is_leap_year(year)
    ...
    --      unilib.calendar_leap_year_flag), false if not
...
local function is_anniversary(pname, year, month, this_day, this_epoch_day)
    ...
    local birth_day = unilib.player_stats_bio_table[pname]["birth_epoch"]
    ...
    local death_day = unilib.player_stats_bio_table[pname]["death_epoch"]
...
local function get_days_in_month(year, month)
    ...
    if unilib.calendar_leap_year_flag and leap_flag and month == 2 then
...
local function update_weekday_offset_list()
    ...
    -- Called by unilib.customise_calendar_weekdays() and unilib.customise_calendar_months()
    -- Updates the offset table used in calls to unilib.get_day_of_week()
    ...
    for i = 0, (month_count - 2) do
        ...
        if unilib.calendar_leap_year_flag and i == 1 then
...
local function get_month_formspec(pname, year, month, day)
    ...
    local current_epoch_day = minetest.get_day_count()
    local current_year, current_month, current_day =
            unilib.convert_minetest_epoch(current_epoch_day)
    ...
    -- Month, year
    formspec = formspec .. "label[0,0;" .. month_name_list[month - 1] .. " " .. year .. "]"
    ...
    if year < unilib.calendar_year_offset or
            (year == unilib.calendar_year_offset and month < unilib.calendar_month_offset) then
    ...
    for i = 0, (weekday_count - 1) do
        ...
        formspec = formspec .. "label[" .. (i + 0.1 + grid_gap) .. ",0.5;" ..
                weekday_short_name_list[column] .. "]"

    end

    -- Calculate the epoch day of the first day, then we can just increment the value
    local init_epoch_day = unilib.deconvert_minetest_epoch(year, month, 1)
    ...
    --      column is specified by unilib.calendar_week_start_offset
    local x = unilib.get_day_of_week(year, month, 1) - unilib.calendar_week_start_offset
    if x < 0 then
        x = weekday_count + x
    end
    ...
    for this_day = 1, get_days_in_month(year, month) do
        ...
        if unilib.calendar_enable_tooltips_flag then

            date_msg = unilib.format_date(
                year,
                month,
                this_day,
                unilib.deconvert_minetest_epoch(year, month, this_day)
            )
        ...
        if unilib.player_stats_bio_table[pname]["event_table"] ~= nil then

            local event_list = unilib.player_stats_bio_table[pname]["event_table"][long_date_code]
        ...
        if unilib.calendar_enable_tooltips_flag then
            ...
            -- N.B. The call to unilib.contract_long_lines() removes the initial newline character
            --      at the start of "tooltip_msg", so we must include a new one
            local combi_msg = minetest.formspec_escape(date_msg)
            if tooltip_msg ~= "" then

                combi_msg = combi_msg .. "\n" ..
                        minetest.formspec_escape(unilib.contract_long_lines(tooltip_msg, 65))
    ...
    if (init_epoch_day - days_in_leap_year) > 0 then
        ...
        formspec = formspec .. "button[" .. button_gap .. "," .. (calendar_grid_height + 1) ..
                ";1,1;prev_year;<<]" .. "tooltip[prev_year;" ..
                minetest.formspec_escape(S("Previous year")) .. "]"
    ...
    if init_epoch_day > 0 then
        ...
        formspec = formspec .. "button[" .. button_gap + 1 .. "," .. (calendar_grid_height + 1) ..
                ";1,1;prev_month;<]" .. "tooltip[prev_month;" ..
                minetest.formspec_escape(S("Previous month")) .. "]"

    end

    formspec = formspec .. "button[" .. button_gap + 2 .. "," .. (calendar_grid_height + 1) ..
            ";1.5,1;this_month;" .. S("Today") .. "]" .. "tooltip[this_month;" ..
            minetest.formspec_escape(S("Today")) .. "]"

    formspec = formspec .. "button[" .. button_gap + 3.5 .. "," .. (calendar_grid_height + 1) ..
            ";1,1;next_month;>]" .. "tooltip[next_month;" ..
            minetest.formspec_escape(S("Next month")) .. "]"

    formspec = formspec .. "button[" .. button_gap + 4.5 .. "," .. (calendar_grid_height + 1) ..
            ";1,1;next_year;>>]" .. "tooltip[next_year;" ..
            minetest.formspec_escape(S("Next year")) .. "]"
...
local function get_day_formspec(pname, day)
    ...
    local formspec = "size[6,5]" ..
            "textarea[0.25,0;6,1;;;" .. minetest.formspec_escape(date_msg) .. "]" ..
            "textarea[0.25,1;6,3.75;;;" .. minetest.formspec_escape(tooltip_msg) .. "]" ..
            "button[4.5,4.3;1.5,1;day_back;" .. S("Back") .. "]tooltip[day_back;" ..
                    minetest.formspec_escape(S("Go back to the calendar")) .. "]"
...
function unilib.calendar_on_receive_fields(player, formname, fields, full_name)
    ...
    local year, month
    local current_year, current_month, current_day =
                unilib.convert_minetest_epoch(minetest.get_day_count())
    ...
    for i = 1, month_length_list[month - 1] do
        ...
        if fields["day_" .. i] ~= nil then
            ...
            minetest.show_formspec(pname, full_name, get_day_formspec(pname, i))
            return
    ...
    minetest.show_formspec(pname, full_name, get_month_formspec(pname, year, month, nil))
...
function unilib.customise_calendar_weekdays(data_list)
    ...
    if unilib.calendar_week_start_offset > (weekday_count - 1) then
        unilib.calendar_week_start_offset = weekday_count - 1
    end

    weekday_column_list = {}
    local this_column = unilib.calendar_week_start_offset
...
function unilib.customise_calendar_months(data_list)
    ...
    if unilib.calendar_leap_year_flag then
...
function unilib.customise_calendar_holidays(data_list)
...
function unilib.get_day_of_week(year, month, day)
...
function unilib.convert_minetest_epoch(epoch_day)

    -- Converts the number of days since the world was created (a value returned by
    --      minetest.get_day_count(), with the first day returning 0), into a date in years, months
    --      and days, taking into account unilib's calendar offsets
    --
    -- e.g. for January 1st, 2020
    --      unilib.calendar_year_offset = 2020
    --      unilib.calendar_month_offset = 1
    --      unilib.calendar_day_offset = 1
    --      minetest.get_day_count() returns 2 (the 3rd day of the world's existence)
    ...
    local year = unilib.calendar_year_offset
    local month = unilib.calendar_month_offset
    local day = unilib.calendar_day_offset
...
function unilib.deconvert_minetest_epoch(year, month, day)

    -- Converts a date expressed as a year (1+), month (e.g. 1-12), day (e.g. 1-31) into the
    --      equivalent Minetest epoch
    -- Note that this function could return a negative epoch (which minetest.get_gametime() never
    --      does)

    -- Start counting from the first day of the month
    local epoch_days = (unilib.calendar_day_offset * -1) + 1

    local this_year = unilib.calendar_year_offset
    local this_month = unilib.calendar_month_offset
    local _
...
function unilib.format_date(year, month, day, epoch_day, hour, minute, second, format_string)
    ...
    local date_string = format_string or unilib.calendar_format_string
    local day_of_week = unilib.get_day_of_week(year, month, day)
    ...
    date_string = string.gsub(date_string, "%%S", unilib.calendar_system)
...
function unilib.open_simple_calendar(clicker)
    ...
    local date_string = unilib.format_date(year, month, day, epoch_day)

    local pname = clicker:get_player_name()
    minetest.chat_send_player(pname, S("Today is @1.", tostring(date_string)))

end

function unilib.open_fancy_calendar(clicker, full_name)
    ...
    local pname = clicker:get_player_name()
    minetest.show_formspec(pname, full_name, get_month_formspec(pname))
...
unilib.customise_calendar_weekdays(weekday_setup_list)
unilib.customise_calendar_months(month_setup_list)
unilib.customise_calendar_holidays(holiday_setup_list)

-- Minetest settings must be positive integers; correct any invalid values
if unilib.calendar_year_offset < 1 then
    unilib.calendar_year_offset = 1
end

if unilib.calendar_month_offset < 1 or unilib.calendar_month_offset > month_count then
    unilib.calendar_month_offset = 1
end

if unilib.calendar_day_offset < 1 or
        get_days_in_month(unilib.calendar_year_offset, unilib.calendar_month_offset) <
        unilib.calendar_day_offset then
    unilib.calendar_day_offset = 1
end
