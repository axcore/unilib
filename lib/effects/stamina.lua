...
local function stamina_log(level, msg, ...)
    ...
    return unilib.log(level, ("[%s] %s"):format(unilib.name, msg:format(...)))
...
local function poison_tick(player_name, ticks, interval, elapsed)
    ...
    if not player or not unilib.stamina_is_poisoned(player) then

        return

    elseif elapsed > ticks then

        unilib.stamina_set_poisoned(player, false)
            ...
            local can_sprint = (
                controls.aux1 and
                        ...
                        (
                            ...
                            not minetest.check_player_privs(player, {fast = true})
                        ) and
                        unilib.stamina_set_saturation(player) > settings_table.sprint_lvl
            )

            if can_sprint then

                unilib.stamina_set_sprinting(player, true)
                ...
                unilib.stamina_set_sprinting(player, false)
...
local function stamina_tick()
    ...
    for _, player in ipairs(minetest.get_connected_players()) do
        ...
        local saturation = unilib.stamina_get_saturation(player)
        if saturation > settings_table.tick_min then
            unilib.stamina_update_saturation(player, saturation - 1)
        end
...
local function health_tick()
    ...
    for _, player in ipairs(minetest.get_connected_players()) do
        ...
        local should_heal = (
            ...
            hp > 0 and
            air > 0 and
            not unilib.stamina_is_poisoned(player)
        )
        ...
        if should_heal then
            ...
            unilib.stamina_exhaust_player(player, settings_table.exhaust_lvl, exhaustion_table.heal)
...
local function show_eat_particles(player, itemname)
    ...
    local def_table = minetest.registered_items[itemname]
    ...
    minetest.add_particlespawner(particle_def_table)
...
function unilib.stamina_get_saturation(player)

    -- Adapted from stamina, was stamina.get_saturation()
    -- Gets a player's saturation

    return tonumber(unilib.get_player_attribute(player, player_attribute_table.saturation))

end

function unilib.stamina_set_saturation(player, level)

    -- Adapted from stamina, was stamina.set_saturation()
    -- Sets a player's saturation
    -- Updates the HUD to reflect saturation level

    unilib.set_player_attribute(player, player_attribute_table.saturation, level)
    player:hud_change(
        get_hud_id(player),
        "number",
...
function unilib.stamina_register_on_update_saturation(func)
...
function unilib.stamina_update_saturation(player, level)
    ...
    local old = unilib.stamina_get_saturation(player)
    ...
    if old < settings_table.heal_lvl and
            not minetest.check_player_privs(player, {interact = true}) then
        return
    end

    unilib.stamina_set_saturation(player, level)

end

function unilib.stamina_change_saturation(player, change)
    ...
    local level = unilib.stamina_get_saturation(player) + change
    level = math.max(level, 0)
    level = math.min(level, settings_table.visual_max)
    unilib.stamina_update_saturation(player, level)
...
function unilib.stamina_is_poisoned(player)

    -- Adapted from stamina, was stamina.is_poisoned()
    -- Checks if a player is poisoned

    return unilib.get_player_attribute(player, player_attribute_table.poisoned) == "yes"

end

function unilib.stamina_set_poisoned(player, poisoned)
    ...
    if poisoned then
        ...
        unilib.set_player_attribute(player, player_attribute_table.poisoned, "yes")

    else

        player:hud_change(hud_id, "text", "unilib_hud_hunger_fg.png")
        unilib.set_player_attribute(player, player_attribute_table.poisoned, "no")

    end

end

function unilib.stamina_register_on_poison(func)
...
function unilib.stamina_poison(player, ticks, interval)
    ...
    unilib.stamina_set_poisoned(player, true)
...
function unilib.stamina_get_exhaustion(player)
    ...
    return tonumber(unilib.get_player_attribute(player, player_attribute_table.exhaustion))

end

function unilib.stamina_set_exhaustion(player, exhaustion)

    -- Adapted from stamina, was stamina.set_exhaustion()
    -- Sets a player's exhaustion level

    unilib.set_player_attribute(player, player_attribute_table.exhaustion, exhaustion)

end

function unilib.stamina_register_on_exhaust_player(func)
...
function unilib.stamina_exhaust_player(player, change, cause)
    ...
    local exhaustion = unilib.stamina_get_exhaustion(player) or 0
    ...
    if exhaustion >= settings_table.exhaust_lvl then
        ...
        unilib.stamina_change_saturation(player, -1)

    end

    unilib.stamina_set_exhaustion(player, exhaustion)
...
function unilib.stamina_register_on_sprinting(func)
...
function unilib.stamina_set_sprinting(player, sprinting)
    ...
    for _, func in ipairs(registered_on_sprinting_table) do
        ...
        local rv = func(player, sprinting)
        if rv == true then
            return
        end
    ...
    if settings_table.sprint_particles and sprinting then

        local pos = player:get_pos()
        local node = minetest.get_node({x = pos.x, y = pos.y - 1, z = pos.z})
        local def_table = minetest.registered_nodes[node.name] or {}
...
if unilib.hunger_enable_flag and minetest.settings:get_bool("enable_damage") then
    ...
    settings_table = {
        eat_particles = true,
        sprint = unilib.hunger_on_sprint_flag,
        ...
        move_tick = 0.5,
        poison_tick = 2.0,
        exhaust_dig = 3 * unilib.hunger_factor,
        exhaust_place = 1 * unilib.hunger_factor,
        exhaust_move = 1.5 * unilib.hunger_on_sprint_factor,
        exhaust_jump = 5 * unilib.hunger_on_sprint_factor,
        exhaust_craft = 20 * unilib.hunger_factor,
        exhaust_punch = 40 * unilib.hunger_factor,
        exhaust_sprint = 28 * unilib.hunger_factor,
        exhaust_lvl = 160,
        heal = 1,
        heal_lvl = 5,
        starve = 1,
    ...
    -- Cause of player's stamina being reduced, in calls to unilib.stamina_exhaust_player()
    exhaustion_table = {
        craft = "craft",
        dig = "dig",
        heal = "heal",
    ...
    -- (Note that the old function is not required any more, but it's good form to store it
    --      somewhere)
    ...
    function minetest.do_item_eat(hp_change, replace_with_item, itemstack, player, pointed_thing)
        ...
        local level = unilib.stamina_get_saturation(player) or 0
        ...
        minetest.sound_play("unilib_eat", {to_player = player:get_player_name(), gain = 0.7})

        if hp_change > 0 then

            unilib.stamina_change_saturation(player, hp_change)
            unilib.stamina_set_exhaustion(player, 0)

        else

            -- Assume hp_change < 0
            unilib.stamina_poison(player, -hp_change, settings_table.poison_tick)
    ...
    minetest.register_on_joinplayer(function(player)
        ...
        local level = unilib.stamina_get_saturation(player) or settings_table.visual_max
        local id = player:hud_add({
            ...
            text2 = "unili_hud_stamina_bg.png",
        })
    ...
    minetest.register_on_placenode(function(pos, oldnode, player, ext)
        unilib.stamina_exhaust_player(player, settings_table.exhaust_place, exhaustion_table.place)
    end)

    minetest.register_on_dignode(function(pos, oldnode, player, ext)
        unilib.stamina_exhaust_player(player, settings_table.exhaust_dig, exhaustion_table.dig)
    end)

    minetest.register_on_craft(function(itemstack, player, old_craft_grid, craft_inv)
        unilib.stamina_exhaust_player(player, settings_table.exhaust_craft, exhaustion_table.craft)
    end)
    ...
    minetest.register_on_respawnplayer(function(player)
        unilib.stamina_update_saturation(player, settings_table.visual_max)
    end)
