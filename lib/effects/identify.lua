...
local function destrange(str)
    ...
    if not unilib.identify_optimise_hud_flag then
...
local function get_node_tiles(node_name)
    ...
    if not unilib.identify_optimise_hud_flag then
        ...
        local mod_name, item_name = unilib.split_name(node_name)

        if node.inventory_image:sub(1, 14) == "[inventorycube" then

            return node.inventory_image .. "^[resize:146x146", "node",
                    minetest.registered_nodes[node_name]

        elseif node.inventory_image ~= "" then

            return node.inventory_image .. "^[resize:16x16", "craft_item",
                    minetest.registered_nodes[node_name]
            ...
            return inventorycube(tiles[1], tiles[6], tiles[3]), "node",
                    minetest.registered_nodes[node_name]
...
local function show_background(player)
    ...
    player:hud_change(
        unilib.get_player_attribute(player, "identify_background_left"),
        "text",
        "unilib_hud_identify_left.png"
    )

    player:hud_change(
        unilib.get_player_attribute(player, "identify_background_middle"),
        "text",
        "unilib_hud_identify_middle.png"
    )

    player:hud_change(
        unilib.get_player_attribute(player, "identify_background_right"),
        "text",
...
local function show(player, form_view, node_description, node_name, item_type, mod_name)
    ...
    if unilib.get_player_attribute(player, "identify_pointed_thing") == "ignore" then
        show_background(player)
    end

    unilib.set_player_attribute(player, "identify_pointed_thing", node_name)

    -- (Show the unilib package name rather than the mod name, if required)
    if unilib.identify_package_name_flag and
            mod_name == unilib.name and
            unilib.node_parent_table[node_name] ~= nil then

        mod_name = unilib.node_parent_table[node_name]

        -- (Switch to yellow label)
        player:hud_change(
            unilib.get_player_attribute(player, "identify_mod"),
            "number",
            colour_yellow
        )
    ...
    player:hud_change(
        unilib.get_player_attribute(player, "identify_image"),
        "text",
        form_view
    )

    player:hud_change(
        unilib.get_player_attribute(player, "identify_name"),
        "text",
        node_description
    )
    ...
    if unilib.get_player_attribute(player, "identify_item_type_in_pointer") ~= item_type then

        local scale = {}

        unilib.set_player_attribute(player, "identify_item_type_in_pointer", item_type)
...
local function show_unknown(player, node_name)
    ...
    unilib.set_player_attribute(player, "identify_pointed_thing", "unknown")
    ...
    if unilib.get_player_attribute(player, "identify_item_type_in_pointer") ~= "node" then

        local scale = {}

        unilib.set_player_attribute(player, "identify_item_type_in_pointer", "node")
...
local function unshow(player)
    ...
    unilib.set_player_attribute(player, "identify_pointed_thing", "ignore")
...
function unilib.toggle_identify_mode(name)
    ...
    if identify_table.player_set_table[name] then
        ...
        unilib.set_player_attribute(player, "identify_enabled", "")
        ...
        if unilib.identify_optimise_hud_flag then
            return S("HUD disabled, punch an item to identify it.")
        end

    else

        unilib.set_player_attribute(player, "identify_enabled", "true")
...
if unilib.identify_enable_hud_flag then
    ...
    minetest.register_on_joinplayer(function(player)
        ...
        unilib.set_player_attribute(player, "identify_background_left", background_id_left)
        unilib.set_player_attribute(player, "identify_background_middle", background_id_middle)
        unilib.set_player_attribute(player, "identify_background_right", background_id_right)
        unilib.set_player_attribute(player, "identify_image", image_id)
        unilib.set_player_attribute(player, "identify_name", name_id)
        unilib.set_player_attribute(player, "identify_mod", mod_id)
        unilib.set_player_attribute(player, "identify_pointed_thing", "ignore")
        unilib.set_player_attribute(player, "identify_item_type_in_pointer", "node")

        -- N.B. Change to original code: the HUD is not necessarily visible on connection
        if unilib.get_player_attribute(player, "identify_enabled") == "true" then
    ...
    if not unilib.identify_optimise_hud_flag then
        ...
        minetest.register_globalstep(function()
            ...
            for _, player in ipairs(identify_table.player_table) do
                ...
                if identify_table.player_set_table[player:get_player_name()] then
                    node = unilib.get_pointed_node(player)
                end
                ...
                if node then
                    ...
                    if unilib.get_player_attribute(player, "identify_pointed_thing") ~=
                            node_name then
                        ...
                        local mod_name = unilib.get_mod_name(node_name)
        ...
        minetest.register_on_punchnode(function(pos, node, player, pointed_thing)
            ...
            if identify_table.player_set_table[player:get_player_name()] then
                ...
                local node_name = node.name
                if unilib.get_player_attribute(player, "identify_pointed_thing") ~= node_name then
