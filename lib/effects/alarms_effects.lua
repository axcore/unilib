...
local epoch_day = minetest.get_day_count() or 0
...
local function activate_alarm(pname, epoch_day, hour, minute)
    ...
    local date_string = unilib.format_date(year, month, day, epoch_day, hour, minute)
    minetest.chat_send_player(pname, S("Today is @1.", tostring(date_string)))
    minetest.sound_play("unilib_alarm", {to_player = pname, gain = 1.0}, true)
...
function unilib.cancel_expired_alarm(pname)
...
if unilib.alarm_enable_flag then
    ...
    minetest.register_globalstep(function(dtime)
        ...
        if timer >= timer_tick then
            ...
            local new_epoch_day = minetest.get_day_count()
            ...
            local game_time, game_hour, game_minute = unilib.get_clock_time()
            ...
            for _, player in ipairs(minetest.get_connected_players()) do
                ...
                if player_table[pname] == nil then
                    ...
                    alarm_time = unilib.get_player_attribute(player, "alarm")
                    if alarm_time ~= nil and alarm_time ~= "" then
                        ...
                        -- Not all combinations of HH:MM actually occur in-game, and a server might
                        --      be subject to some lag, so we have to allow a grace period of 15
                        --      in-game minutes
