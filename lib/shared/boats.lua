...
local function prepare_boat_entity(full_name, part_name)
    ...
    function def_table.on_punch(self, puncher)
        ...
        if not self.driver then
            ...
            if not unilib.is_creative(name) or
...
function unilib.register_simple_boat(data_table)

    -- Adapted from boats/init.lua
    -- Creates a boatfor a tree type already registered in an earlier call to unilib.register_tree()
    --
    -- data_table compulsory fields:
    --      part_name (str): e.g. "apple", a key in unilib.tree_table
    --      orig_name (str): e.g. "boats:boat"
    --
    -- data_table optional fields:
    --      replace_mode (str): e.g. "defer"
    --      burnlevel (int): value in the range 1-5, see ../lib/system/global.lua. Overrides the
    --          value of "burntime" is specified
    --      burntime (int): e.g. 20
    ...
    -- Boats require code imported from minetest_game/player_api, if not already loaded
    ...
    unilib.register_craftitem(full_name, orig_name, replace_mode, {
        ...
        on_place = function(itemstack, placer, pointed_thing)
            ...
            boat = minetest.add_entity(pointed_thing.under, entity_name)
            if boat then
