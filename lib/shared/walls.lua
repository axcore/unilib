...
function unilib.register_wall(data_table)
    ...
    --      sound_name (str): e.g. "stone", a key in unilib.sound_table
    ...
    if not unilib.add_walls_flag then
        return nil
    end
    ...
    unilib.register_node(full_name, orig_name, replace_mode, {
        ...
        connects_to = {"group:fence", "group:stone", "group:wall"},
        drawtype = "nodebox",
    ...
    --      unregistered here, rather than in ../lib/system/register.lua (as is the case for stairs)
    ...
    if (replace_mode == "replace" or replace_mode == "hide") and
             minetest.registered_nodes[orig_name] ~= nil then
