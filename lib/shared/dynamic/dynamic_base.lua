...
local function extend_me(pos, wherein, output, sides, chance)
    ...
    --      (and optionally surrounding nodes matching "wherein") into the "output" node
...
local function flood_me(pos, wherein, output, sides, size, ore_table, top_table)
    ...
    -- Adapted from underch/dynamic.lua
    -- Called from the LBM created by unilib.dynamic.register_flooder(), to convert a dynamic node
    --      (and optionally surrounding nodes matching "wherein") into the "output" node
    ...
    while i <= pc do
        ...
        if core.get_node(positions[i]).name == wherein and math.random() < size/blocks then
            ...
            if not blockset then
                core.set_node(positions[i], {name = output})
            end
...
local function flood_me_jit(pos, wherein, output, sides, size, ore_table, top_table)
    ...
    while i <= pc do
        ...
        elseif is_jit then
            ...
            unilib.jbiomes.reveal(positions[i], true)
...
function unilib.dynamic.register_extender(data_table)

    -- Adapted from underch/dynamic.lua
    -- Creates a dynamic node, which is placed randomly in the map (from a package like
    --      "mapgen_underch", calling a function like unilib.pbiomes.ore() )
    ...
    unilib.register_node(
        ...
        {
            description = unilib.utils.brackets(S("Dynamic extender node"), data_table.label),
...
function unilib.dynamic.register_flooder(data_table)
    ...
    unilib.register_node(
        ...
        data_table.replace_mode,
        {
            description = unilib.utils.brackets(S("Dynamic flooder node"), data_table.label),
            tiles = {"unilib_dynamic_node.png"},
             groups = {not_in_creative_inventory = 1},
            -- (no sounds)
...
function unilib.dynamic.register_flooder_jit(data_table)
