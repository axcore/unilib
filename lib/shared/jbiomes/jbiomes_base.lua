...
---------------------------------------------------------------------------------------------------
-- jbiomes_base.lua
--      Set up shared functions for creating JIT pseudo-biomes
---------------------------------------------------------------------------------------------------

local S = unilib.intllib

-- When placing ore distributions in unilib.jbiomes.scatter() and unilib.jbiomes.blob(), introduce
--      randomness
local x_off = math.random()
local y_off = math.random()
local z_off = math.random()

-- The number of biomes specified in the calls to unilib.jbiomes.setup_matrix() and
--      unilib.jbiomes.setup_converted (upper limit: unilib.constant.underch_max_biomes)
local biome_count = 0

-- A table used to convert matrix nodes into pseudo-biome appropriate items (see the comments in
--      unilib.jbiomes.setup_converted() )
...
-- Set to the return values of core.get_content_id(), during a call to unilib.setup_matrix()
local c_bulk = nil
local c_crust = nil
local c_stone = nil

-- unilib.jbiomes.reveal() calls check_neighbours(), which in turn calls unilib.jbiomes.reveal().
...
-- For examples of how to create JIT pseudo-biomes, see the "mapgen_underch_jit" and
--      "mapgen_jades_jit" packages
--
...
--      - The first stage is triggered by a call to unilib.jbiomes.setup_matrix(), which (among
--          other things)  converts all "unilib:stone_ordinary" nodes to matrix nodes; "crust" if
--          exposed to the air, "bulk" if not. Some ornaments such as mushrooms can also be placed
--          at this stage
--      - The second stage is triggered by a call to unilib.jbiomes.setup_converted(), which handles
--          matrix nodes when they are exposed to the air
--      - An individual matrix node, when exposed to the air, triggers a call to
--          unilib.jbiomes.reveal(). This replaces the node with a stone that's characteristic of
--          the pseudo-biome, but will sometimes place an alternative node, creating ores
--      - unilib.jbiomes.reveal() will call itself recursively in order to handle conversion of any
...
local function check_neighbours(pos, crust_flag)
    ...
    while i <= pc do
        ...
        local name = core.get_node(positions[i]).name
        if name == "unilib:misc_matrix_bulk" or name == "unilib:misc_matrix_crust" then

            if unilib.jbiomes.reveal(positions[i], false, crust_flag) and (not crust_flag) then
                add_pos(positions[i])
            end
...
function unilib.jbiomes.dig_shadow(pos, oldnode, oldmetadata, digger)
...
function unilib.jbiomes.scatter(pos, y_min, y_max, node1, node2, scarcity, period, period2, period3)
    ...
    if node1 ~= nil and core.get_node(pos).name ~= node1 then
        return
    end
...
function unilib.jbiomes.blob(pos, y_min, y_max, node1, node2, size2, period, period2, period3)
...
function unilib.jbiomes.is_shadow(pos)

    -- Adapted from underch/jit.lua, underch.jit.gc()

    local node = core.get_node(pos).name
    if node == "ignore" or core.registered_nodes[node] == nil then
        return false
    end

    local value = core.registered_nodes[node].groups.jit_shadow
    return value == nil or value == 0

end

function unilib.jbiomes.is_visible(pos)

    -- Adapted from underch/jit.lua, underch.jit.is_visible()

    return unilib.jbiomes.is_shadow({x = pos.x + 1, y = pos.y, z = pos.z}) or
            unilib.jbiomes.is_shadow({x = pos.x, y = pos.y + 1, z = pos.z}) or
            unilib.jbiomes.is_shadow({x = pos.x, y = pos.y, z = pos.z + 1}) or
            unilib.jbiomes.is_shadow({x = pos.x - 1, y = pos.y, z = pos.z}) or
            unilib.jbiomes.is_shadow({x = pos.x, y = pos.y - 1, z = pos.z}) or
            unilib.jbiomes.is_shadow({x = pos.x, y = pos.y, z = pos.z - 1})

end

function unilib.jbiomes.reveal(pos, recursion_flag, crust_flag)

    -- Adapted from underch/jit.lua, underch.jit.reveal()

    -- Basic checks
    if not unilib.jbiomes.is_visible(pos) then
        return false
    end

    -- Place ore-distributions around this position
    for _, t in ipairs(ore_blob_table) do
        unilib.jbiomes.blob(pos, t[1], t[2], t[3], t[4], t[5], t[6], t[7], t[8])
    end

    for _, t in ipairs(ore_scatter_table) do
        unilib.jbiomes.scatter(pos, t[1], t[2], t[3], t[4], t[5], t[6], t[7], t[8])
    end

    -- Find the pseudo-biome for this position, specified in the earlier call to
    --      unilib.jbiomes.setup_converted()
    local darkness = core.get_perlin(unilib.pbiomes.np_darkness):get_3d(pos)
    local water = core.get_perlin(unilib.pbiomes.np_water):get_3d(pos)
    local pressure = unilib.pbiomes.get_pressure(
        pos.y,
        core.get_perlin(unilib.pbiomes.np_pressure):get_3d(pos)
    )
    ...
    if biome < 1 or biome > biome_count then
        ...
        if biome > unilib.constant.underch_max_biomes then
            unilib.utils.show_warning("unilib.jbiomes.reveal(): Invalid pseudo-biome", biome)
        end
    ...
    ---     underch code)
    if recursion_flag or (crust_flag and unilib.jbiomes.is_shadow(pos)) then
...
function unilib.jbiomes.setup_matrix(biome_table)
    ...
    if called_flag then
        ...
        unilib.utils.show_warning(
            "unilib.jbiomes.setup_matrix() Multiple calls to this function may have undesirable" ..
                    " results"
        )
    ...
    c_bulk = core.get_content_id("unilib:misc_matrix_bulk")
    c_crust = core.get_content_id("unilib:misc_matrix_crust")
                ...
                for x = x0, x1 do
                    ...
                    if biome < 1 or biome > biome_count then
                        ...
                        if biome > unilib.constant.underch_max_biomes then
                            ...
                            unilib.utils.show_warning(
                                "unilib.jbiomes.setup_matrix(): Invalid pseudo-biome", biome
                            )
...
function unilib.jbiomes.replace_in_matrix(vi, data, def1, def2)
...
function unilib.jbiomes.setup_converted(biome_table, blob_table, scatter_table)
    ...
    if #biome_table ~= biome_count then
        ...
        unilib.utils.show_warning(
            "unilib.jbiomes.setup_converted(): Pseudo-biome mismatch", #biome_table, biome_count
        )

    end

    -- These table are used by unilib.jbiomes.reveal()
