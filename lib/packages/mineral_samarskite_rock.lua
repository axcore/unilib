---------------------------------------------------------------------------------------------------
-- unilib mod by A S Lewis, incorporating materials from many other mods
---------------------------------------------------------------------------------------------------
-- From:    AATO
-- Code:    GPL 3.0
-- Media:   GPL 3.0
---------------------------------------------------------------------------------------------------

unilib.pkg.mineral_samarskite_rock = {}

local S = unilib.intllib
local mode = unilib.imported_mod_table.aato.add_mode

---------------------------------------------------------------------------------------------------
-- New code
---------------------------------------------------------------------------------------------------

function unilib.pkg.mineral_samarskite_rock.init()

    return {
        description = "Samarskite rock (as mineral)",
        notes = "An alternative to the stone ores with samarskite",
        depends = "mineral_samarskite",
    }

end

function unilib.pkg.mineral_samarskite_rock.exec()

    local c_lump = "unilib:mineral_samarskite_lump"
    local hardness = unilib.mineral_table["samarskite"]["hardness"]

    unilib.register_mineral_rock("samarskite")

    unilib.register_node("unilib:mineral_samarskite_rock", nil, mode, {
        -- Texture from AATO, samarskite_y.png. Original code
        description = S("Samarskite Rock"),
        tiles = {"unilib_mineral_samarskite_rock.png"},
        groups = {cracky = 1},
        unilib.sound_table.stone,

        drop = {
            max_items = 1,
            items = {
                {items = {"unilib:mineral_samarskite_rock"}, rarity = (100 * hardness)},
                {items = {c_lump .. " 2"}, rarity = (2 + hardness)},
                {items = {c_lump}},
            },
        },
    })

end
