---------------------------------------------------------------------------------------------------
-- unilib mod by A S Lewis, incorporating materials from many other mods
---------------------------------------------------------------------------------------------------
-- From:    real_minerals
-- Code:    GPL 3.0
-- Media:   unknown
---------------------------------------------------------------------------------------------------

unilib.pkg.metal_silver_sterling = {}

local S = unilib.intllib
local mode = unilib.imported_mod_table.real_minerals.add_mode

---------------------------------------------------------------------------------------------------
-- New code
---------------------------------------------------------------------------------------------------

function unilib.pkg.metal_silver_sterling.init()

    return {
        description = "Sterling silver",
        depends = {"metal_copper_real", "metal_silver_real"},
    }

end

function unilib.pkg.metal_silver_sterling.exec()

    unilib.register_metal({
        -- Original to unilib
        part_name = "silver_sterling",
        description = S("Sterling Silver"),

        alloy_flag = true,
        hardness = 2,
    })

    unilib.register_metal_powder({
        -- Original to unilib. Creates unilib:metal_silver_sterling_powder
        part_name = "silver_sterling",
        orig_name = nil,

        replace_mode = mode,
        description = S("Sterling Silver Powder"),
        no_lump_flag = true,
    })

    unilib.register_craftitem(
        -- From real_minerals:sterling_silver_ingot
        "unilib:metal_silver_sterling_ingot",
        "real_minerals:sterling_silver_ingot",
        mode,
            {
            description = S("Sterling Silver Ingot"),
            inventory_image = "unilib_metal_silver_sterling_ingot.png",
            -- N.B. alloy = 1 not in original code
            groups = {alloy = 1, metal_ingot = 1},
        }
    )
    unilib.register_craft({
        -- From real_minerals:sterling_silver_ingot
        type = "shapeless",
        output = "unilib:metal_silver_sterling_ingot 4",
        recipe = {
            "unilib:metal_silver_real_ingot",
            "unilib:metal_silver_real_ingot",
            "unilib:metal_silver_real_ingot",
            "unilib:metal_copper_real_ingot",
        },
    })
    unilib.register_craft({
        -- Original to unilib
        output = "unilib:metal_silver_sterling_ingot 9",
        recipe = {
            {"unilib:metal_silver_sterling_block"},
        }
    })

    unilib.register_node(
        -- From real_minerals:sterling_silver_block
        "unilib:metal_silver_sterling_block",
        "real_minerals:sterling_silver_block",
        mode,
        {
            description = S("Sterling Silver Block"),
            tiles = {"unilib_metal_silver_sterling_block.png"},
            groups = {bendy = 2, cracky = 2, level = 2, melty = 2, snappy = 1},
            sounds = unilib.sound_table.metal,

            -- N.B. true in original code
            is_ground_content = false,
        }
    )
    unilib.register_craft_3x3({
        -- From real_minerals:sterling_silver_block
        output = "unilib:metal_silver_sterling_block",
        ingredient = "unilib:metal_silver_sterling_ingot",
    })
    unilib.register_stairs("unilib:metal_silver_sterling_block")

end
