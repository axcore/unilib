...
blacklist_table["unilib:tnt_ordinary_placeholder"] = true
blacklist_table["tnt:boom"] = true

blacklist_table["unilib:tnt_gunpowder"] = true
blacklist_table["tnt:gunpowder"] = true

blacklist_table["unilib:tnt_gunpowder_burning"] = true
blacklist_table["tnt:gunpowder_burning"] = true

blacklist_table["unilib:tnt_ordinary"] = true
blacklist_table["tnt:tnt"] = true
-- (From "protector_redo" mod) Prevent accidental replacement of your protector blocks
blacklist_table["protector:protect"] = true
blacklist_table["protector:protect2"] = true
...
for i, v in ipairs(mode_description_list) do
    mode_description_list[i] = minetest.formspec_escape(v)
end
...
local function convert_turf_to_dirt(turf_name)
    ...
    local def_table = minetest.registered_nodes[turf_name]
...
---------------------------------------------------------------------------------------------------
-- Local functions (from replacer/check_owner.lua)
---------------------------------------------------------------------------------------------------

local function is_owned(pos, placer)
    ...
    if type(minetest.is_protected) == "function" then
        ...
        local result = minetest.is_protected(pos, pname)
        if result then
...
local function get_human_readable_pattern(pattern)
    ...
    local def_table = minetest.registered_nodes[node_name]
...
local function get_formspec(player_name, current_pattern, player)
    ...
    --[[
    local in_creative_mode = minetest.settings:get_bool("creative_mode") or
                minetest.check_player_privs(player_name, {creative = true})
    ]]--
    local in_creative_mode = unilib.is_creative(player_name)
    ...
    -- N.B. Original formspec tweaked for aesthetics and legibility3
    --[[
    ...
    local formspec = "size[18,10]" ..
        "label[6,0;Node Replacement Tool Setup and History]" ..
        "button_exit[8,9.4;2,0.8;quit;Exit]" ..
        "label[0.2,8.3;Note: The selected mode and pattern history are reset when the server" ..
                " restarts.\nNote: The selected mode is valid for all replacement tools you" ..
                " use. The pattern history is valid for this tool only.]" ..
        "label[0.2,0.6;Select mode: When replacing (punching, left-click) or " ..
                "placing (right-click) a block...]" ..
        "dropdown[0.2,1.1;18;select_mode;" ..
                table.concat(mode_description_list, ",") ..
                ";" .. tostring(current_mode) .. ";]" ..
        "label[0.2,2.1;History: Click here to set the tool to a pattern you have used before:]" ..
        "tablecolumns[color;" ..
                "text,align=right,tooltip=Amount of nodes of this type left in your inventory:" ..
                ";color;text,align=left,tooltip=Stored pattern:]" ..
    ...
    for i, v in ipairs(history_table[player_name]) do
        ...
        if not in_creative_mode then
            ...
            if counted_inv[part_list[1]] then
                ...
                amount_left = "#00FF00," .. tostring(counted_inv[part_list[1]]) .. " available:" ..
                        ",#00FF00"
        ...
        hist_entry_table[i] = tostring(amount_left) .. "," ..
                minetest.formspec_escape(get_human_readable_pattern(v) .. " [" .. v .. "]")
...
local function identify_shape_and_material(full_name)
    ...
    elseif mod_name == "unilib" and unilib.stair_deconvert_table[full_name] ~= nil then

        local orig_name = unilib.stair_deconvert_table[full_name]
        suffix = full_name:sub(#orig_name + 1)
        return {"stairs", orig_name, "unilib", "", unilib.get_item_name(orig_name), suffix}

    elseif mod_name == "unilib" and unilib.carving_deconvert_table[full_name] ~= nil then

        local orig_name = unilib.carving_deconvert_table[full_name]
        suffix = full_name:sub(#orig_name + 1)
        return {"carvings", orig_name, "unilib", "", unilib.get_item_name(orig_name), suffix}
...
local function get_new_node_data(old_node, stored_pattern, player_name)
    ...
    if unilib.is_table_empty(user_mode_table) or
    ...
    if user_mode_table[player_name] == "material" then
        ...
        if not minetest.registered_nodes[new_name] and
                alt_name_table[new_name] then
...
local function do_replace(itemstack, user, pointed_thing, mode)
    ...
    if pointed_thing.type ~= "node" then
       ...
       minetest.chat_send_player(name, S("Node replacement error: No node"))
       return nil
    ...
    if node.name and node.name ~= "" and blacklist_table[node.name] then
    ...
    if blacklist_table[part_list[1]] then
    ...
    if node.name == part_list[1] then
       ...
       if node.param1 ~= part_list[2] or node.param2 ~= part_list[3] then
            minetest.add_node(pos, {name = node.name, param1 = part_list[2], param2 = part_list[3]})
       end
    ...
    minetest.add_node(pos, {name = part_list[1], param1 = part_list[2], param2 = part_list[3]})
    -- No item willl be removed from the inventory
    return nil
...
function unilib.pkg.item_replacer.init()
    ...
    return {
        ...
        notes = "SNEAK + right-click to set a replacement node. Left-click to replace a node," ..
                ...
                " but not its shape",
        depends = {"container_chest_ordinary", "dirt_ordinary", "item_stick_ordinary"},
    }

end

function unilib.pkg.item_replacer.exec()

    unilib.register_tool("unilib:item_replacer", "replacer:replacer", mode, {
        -- From replacer:replacer
        description = S("Node Replacement Tool"),
        inventory_image = "unilib_item_replacer.png",
        groups = {},
        ...
        on_place = function(itemstack, placer, pointed_thing)
            ...
            if pointed_thing.type ~= "node" then
                ...
                minetest.chat_send_player(name, S("Node replacement error: No node selected"))
                return nil
    ...
    minetest.register_on_player_receive_fields(function(player, formname, fields)
        ...
        if not formname or formname ~= "unilib:formspec_item_replacer" then
            return false
        end
        ...
        if fields and fields.history_table and history_table and
            ...
            local selected = minetest.explode_table_event(fields.history_table)
