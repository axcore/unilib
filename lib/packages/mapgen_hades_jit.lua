...
-- Before setting any of the flags below to true, you should edit the "hades_jit" remix to uncomment
--      the necessary packages (i.e. by removing the # character at the beginning of each line)
...
local function add_ore(data_table)
    ...
    elseif mineral ~= nil then
        ...
        unilib.utils.show_warning("mapgen_hades_jit package, add_ore(): Invalid arguments")
        return
    ...
    if core.registered_nodes[full_name] ~= nil then
        ...
        if debug_duplicate_ore_flag then
            ...
            unilib.utils.show_warning(
                "mapgen_hades_jit package, add_ore(): Ore already exists", full_name
            )
...
local function distribute_single_ore(data_table)
    ...
    if (wherein ~= "all" and core.registered_nodes["unilib:" .. wherein] == nil) or
        ...
        unilib.utils.show_warning(
            "mapgen_hades_jit package, distribute_single_ore(): Unrecognised node(s)",
            "unilib:" .. wherein,
            "unilib:" .. ore
        )
...
local function distribute_blob(data_table)
    ...
    if (wherein ~= "all" and core.registered_nodes["unilib:" .. wherein] == nil) or
        ...
        unilib.utils.show_warning(
            "mapgen_hades_jit package, distribute_blob(): Unrecognised node(s)",
            "unilib:" .. wherein,
            "unilib:" .. ore
        )
...
local function distribute_scatter(data_table)
    ...
    if (wherein ~= "all" and core.registered_nodes["unilib:" .. wherein] == nil) or
        ...
        unilib.utils.show_warning(
            "mapgen_hades_jit package, distribute_scatter(): Unrecognised node(s)",
            "unilib:" .. wherein,
            "unilib:" .. ore
        )
...
local function place_single_ore(pos, distrib_table, item_name, height_adjust)
    ...
    -- Adapted from unilib.jbiomes.ore() in ../lib/shared/jbiomes/jbiomes_base.lua
    -- That function uses the arguments (pos, node1, node2, probability), but here we assume that
    --      values in "distrib_table" are correct
    ...
    if debug_ore_flag and core.registered_nodes[full_name] == nil then
        ...
        unilib.utils.show_warning(
            "mapgen_hades_jit package, place_single_ore(): Undefined ore", full_name
        )
    ...
    if distrib_table.y_min <= pos.y and
            ...
            math.random() < (
                distrib_table.probability * ore_factor_table[item_name] * height_adjust
            ) then
...
local function place_blob(pos, distrib_table)
    ...
    if debug_ore_flag and core.registered_nodes[full_name] == nil then
        ...
        unilib.utils.show_warning(
            "mapgen_hades_jit package, place_blob(): Undefined blob", full_name
        )
    ...
    if t1 < distrib_table.size then
        ...
        core.set_node(pos, {name = "unilib:" .. distrib_table.ore})
        return true
...
local function place_scatter(pos, distrib_table)
    ...
    if debug_ore_flag and core.registered_nodes[full_name] == nil then
        ...
        unilib.utils.show_warning(
            "mapgen_hades_jit package, place_scatter(): Undefined scatter", full_name
        )
...
local function populate_ore_tables()
    ...
    --      unilib.pkg.mapgen_hades_jit.exec()
    --
    -- N.B. This function is identical to the one in the "mapgen_hades" package, but moving them
...
local function override_nodes()
    ...
    for _, part_name in ipairs(basic_stone_list) do
        ...
        if core.registered_nodes["unilib:stone_" .. part_name .. "_cobble"] ~= nil then

            add_jit_property("unilib:stone_" .. part_name .. "_cobble")
            add_jit_property("unilib:stone_" .. part_name .. "_cobble_mossy")

        elseif core.registered_nodes["unilib:stone_" .. part_name .. "_rubble"] ~= nil then
...
local function setup_lbm()
    ...
    unilib.register_lbm{
        label = "Replace crust with bulk [mapgen_hades_jit]",
        name = "unilib:lbm_mapgen_hades_jit",
...
local function switch_stone(pos, item_name)
    ...
    if name == "unilib:misc_matrix_bulk" or name == "unilib:misc_matrix_crust" then
        ...
        core.set_node(pos, {name = "unilib:" .. item_name})
...
function unilib.pkg.mapgen_hades_jit.init()

    return {
        description = "Pseudo-biomes for the \"hades_jit\" remix",
        notes = "A simplistic implementation of PFAA Geologica's geology system, partly based" ..
                ...
                " This package assumes that all packages listed in the \"hades_jit\" remix have" ..
                " been executed (rather than listing them all here as dependencies)",
        excludes = {"mapgen_hades", "mapgen_underch", "mapgen_underch_jit"},
        depends = "stone_ordinary",
    }

end

function unilib.pkg.mapgen_hades_jit.post()

    -- Load files in ../lib/shared/. Note that ../lib/shared/pbiomes/ is required, even though it's
    ...
    for _, part_name in pairs(basic_stone_list) do
        ...
        if core.registered_nodes["unilib:stone_" .. part_name] == nil then
            ...
            unilib.utils.show_warning(
                "mapgen_hades_jit package, post(): Aborting because of a non-existent node" ..
                        " (\"unilib:stone_" .. part_name .. "\"); this package requires the" ..
                        " \"hades_jit\" remix, or a suitable replacement providing the same" ..
    ...
    if blob_default_flag then
        ...
        table.insert(
            blob_table,
            {unilib.constant.y_min, unilib.constant.y_max, nil, "unilib:sand_silver",
            0.09, 21, 22, 23}
        )
        table.insert(
            blob_table,
        ...
        table.insert(
            blob_table,
            {unilib.constant.y_min, unilib.constant.y_max, nil, "unilib:gravel_ordinary",
            0.09, 21, 19, 24}
        )
        table.insert(
            blob_table,
        ...
        table.insert(
            blob_table,
            {unilib.constant.y_min, unilib.constant.y_max, nil, "unilib:stone_ordinary_cobble",
            0.09, 20, 21, 19}
        )
    ...
    if scatter_default_flag then
        ...
        table.insert(
            scatter_table,
            {unilib.constant.y_min, -500, nil, "unilib:stone_ordinary_with_diamond",
            180, 32, 31, 29}
        )
        table.insert(
            scatter_table,
    ...
    if scatter_moreores_flag then
        ...
        table.insert(
            scatter_table,
            {unilib.constant.y_min, -512, nil, "unilib:stone_ordinary_with_mithril",
            185, 33, 28, 29}
        )
    ...
    if scatter_technic_flag then
        ...
        table.insert(
            scatter_table,
            {unilib.constant.y_min, -80, nil, "unilib:stone_ordinary_with_chromium",
            170, 28, 31, 33}
        )
        table.insert(
            scatter_table,
    ...
    if scatter_xtraores_flag then
        ...
        table.insert(
            scatter_table,
            {unilib.constant.y_min, -100, nil, "unilib:stone_ordinary_with_planexium",
            180, 35, 29, 28}
        )
        table.insert(
            scatter_table,
            {unilib.constant.y_min, -1000, nil, "unilib:stone_ordinary_with_sybilline",
            190, 29, 33, 34}
        )
        table.insert(
            scatter_table,
            {unilib.constant.y_min, -2000, nil, "unilib:stone_ordinary_with_ozymandium",
            195, 28, 30, 32}
        )
        table.insert(
            scatter_table,
            {unilib.constant.y_min, -5000, nil, "unilib:stone_ordinary_with_adamantite",
            200, 31, 32, 33}
        )
        table.insert(
            scatter_table,
            {unilib.constant.y_min, -10000, nil, "unilib:stone_ordinary_with_rarium",
            205, 30, 34, 37}
        )
        table.insert(
            scatter_table,
            {unilib.constant.y_min, -15000, nil, "unilib:stone_ordinary_with_unobtainium",
            210, 29, 37, 34}
        )
        table.insert(
            scatter_table,
            {unilib.constant.y_min, -20000, nil, "unilib:stone_ordinary_with_tritonium",
            215, 32, 29, 37}
        )
        table.insert(
            scatter_table,
            {unilib.constant.y_min, -25000, nil, "unilib:stone_ordinary_with_geminitium",
            220, 32, 33, 35}
        )
    ...
    -- Pass this data to the code in ../lib/shared/jbiomes/jbiomes_base.lua
        ...
        for i = 1, basic_stone_count do
            ...
            if single_ore_table["stone_" .. part_name] == nil and
                ...
                unilib.utils.show_msg(
                    "mapgen_hades_jit package: basic stone has no ores", part_name
                )
