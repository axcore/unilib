...
function unilib.pkg.flower_waterlily_ordinary_alt.exec()
    ...
    if unilib.global.pkg_executed_table["flower_waterlily_ordinary"] ~= nil then

        if core.registered_nodes["unilib:flower_waterlily_ordinary"] ~= nil then
            core.unregister_item("unilib:flower_waterlily_ordinary")
        end

        if core.registered_nodes["flowers:waterlily"] ~= nil then
            core.unregister_item("flowers:waterlily")
        end

        if core.registered_nodes["unilib:flower_waterlily_ordinary_waving"] ~= nil then
            core.unregister_item("unilib:flower_waterlily_ordinary_waving")
        end

        if core.registered_nodes["flowers:waterlily_waving"] ~= nil then
            core.unregister_item("flowers:waterlily_waving")
        end
    ...
    for _, mini_list in ipairs(waterlily_list) do
        ...
        unilib.register_node("unilib:flower_waterlily_ordinary_" .. part_name, orig_name, mode, {
            ...
            on_place = function(itemstack, placer, pointed_thing)
                ...
                if place_pos and not core.is_protected(place_pos, placer:get_player_name()) then
    ...
    unilib.register_decoration_convertable({
        ...
        climate_table = {
            temp_max = unilib.utils.convert_biome_lib_temp(-0.22),
