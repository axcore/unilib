...
unilib.pkg.shared_castle_gates_gateways = {}
...
local function get_dirs(facedir)
    ...
    dirs.back = core.facedir_to_dir(facedir)
...
local function get_buildable_to(pos)
    ...
    -- Adapted from castle_gates/gate_functions.lua
    return core.registered_nodes[core.get_node(pos).name].buildable_to
...
local function get_door_layout(pos, facedir, player)
    ...
    door.previous_move = core.get_meta(pos):get_string("previous_move")
    ...
    while test_pos ~= nil do
        ...
        local test_node = core.get_node(test_pos)
        ...
        if core.is_protected(test_pos, player:get_player_name()) and
                not core.check_player_privs(player, "protection_bypass") then
        ...
        if test_node_def.paramtype2 == "facedir" then
            ...
            if coplanar and test_node_def.groups.castle_gateway == castle_gateway_group_value then
                ...
                for _, dir in pairs(test_directions) do
                    ...
                    local adjacent_node = core.get_node(adjacent_pos)
                    local adjacent_def = core.registered_nodes[adjacent_node.name]

                    -- N.B. Guard against unknown nodes
                    if adjacent_def then

                        -- N.B. Modified original code to prevent an edge section sliding into the
                        --      position of an ordinary section, thereby replacing it
                        --[[
                        ...
                        if adjacent_def.buildable_to == true then
                            can_slide_to:set_pos(adjacent_pos, true)
                        end

                        -- If we ourselves are an edge node, don't look in the direction in which
                        --      we're an edge
                        if test_node_def._gate_edges == nil or
                                not test_node_def._gate_edges[dir] then
                            ...
                            if tested:get_pos(adjacent_pos) == nil then
                                ...
                                if adjacent_def.paramtype2 == "facedir" then
                                    ...
                                    -- Look at the adjacent node's definition. If it's got gate
                                    --      edges, check if they point back at us
                                    if adjacent_def._gate_edges ~= nil then

                                        local adjacent_directions = get_dirs(adjacent_node.param2)
                                        for dir2, val in pairs(adjacent_def._gate_edges) do

                                            if vector.equals(
                                                vector.add(adjacent_pos, adjacent_directions[dir2]),
                                                test_pos
                                            ) then
        ...
        for _, direction in pairs({-1, 1}) do
            ...
            for _, door_node in pairs(door.all) do
                ...
                if not vector.equals(door_node.pos, origin) then
                    ...
                    local newdef = core.registered_nodes[newnode.name]
...
function unilib.pkg.shared_castle_gates_gateways.trigger_gateway(pos, node, player)
    ...
    if door ~= nil then
        ...
        for _, door_node in pairs(door.all) do
            core.set_node(door_node.pos, {name = "air"})
        end
        ...
        for _, door_node in pairs(door.all) do
            ...
            core.set_node(door_node.pos, door_node.node)
            core.get_meta(door_node.pos):set_string("previous_move", door.previous_move)
        ...
        if door_moved then
            ...
            core.after(1, function()
                ...
                unilib.pkg.shared_castle_gates_gateways.trigger_gateway(
                    door.all[1].pos, door.all[1].node, player
                )
...
function unilib.pkg.shared_castle_gates_gateways.init()
