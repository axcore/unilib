...
local function do_trench(minp, maxp, seed, vm, area, param)
    ...
    local c_sample = core.get_content_id("unilib:stone_ordinary")
    local c_air = core.get_content_id("air")

    local n_elv = core.get_perlin_map(np_elv_table, pmapsize) : get_2d_map_flat(minpxz)
    local n_fault = core.get_perlin_map(np_fault_table, pmapsize) : get_2d_map_flat(minpxz)
...
function unilib.pkg.chat_cut_trench.init()

    return {
        description = "Chat command /cut_trench",
        notes = "Cuts deep trenches in newly-generated terrain",
    }

end

function unilib.pkg.chat_cut_trench.exec()

    core.register_chatcommand("cut_trench", {
        params = "[width]",
        description = S(
            "Cuts deep trenches at intervals of 100 blocks in newly-generated terrain (useful" ..
            " for visualising ore distributions). See also the Minetest setting" ..
