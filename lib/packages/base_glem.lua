...
function unilib.pkg.base_glem.init()

    -- By running this section in .init(), rather than .exec, we can guarantee that it is the first
    --      package code run (assuming that the GLEM remixes lists this package first)
    ...
    return {
        description = "Base package for all GLEM remixes",
...
function unilib.pkg.base_glem.post()
