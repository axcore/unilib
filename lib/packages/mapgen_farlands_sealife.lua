...
local function spawn_kelp(pos, kelp_name, count)
    ...
    if math.random(1, 2) == 1 and
            core.get_node({x = pos.x, y = pos.y + (count + 3), z = pos.z}).name ~= "air" then

        core.add_node({x = pos.x, y = pos.y + (count + 1), z = pos.z}, {name = kelp_name})
...
function unilib.pkg.mapgen_farlands_sealife.post()
    ...
    core.register_on_generated(function(minp, maxp)
        ...
        local dirt = core.find_nodes_in_area(minp, maxp, {"unilib:sand_ordinary"})
            ...
            if math.random(1, 50) == 1 then
                ...
                if core.get_node({x = pos.x, y = pos.y + 1, z = pos.z}).name ==
                    ...
                    elseif math.random(1, 3) == 1 then
                        ...
                        core.add_node(
                            {x = pos.x, y = pos.y + 1, z = pos.z},
                            {name = "unilib:plant_anemone_normal"}
                        )
