...
local function tnt_explode(pos, radius, ignore_protection, ignore_on_blast, owner, explode_center)
    ...
    local count = 0
    local c_tnt
    local c_tnt_burning = core.get_content_id("unilib:tnt_ordinary_burning")
    local c_tnt_boom = core.get_content_id("unilib:tnt_ordinary_placeholder")
    local c_air = core.CONTENT_AIR
    local c_ignore = core.CONTENT_IGNORE

    if unilib.global.enable_explosives_flag then
        c_tnt = core.get_content_id("unilib:tnt_ordinary")
    else
    ...
    unilib.utils.log(
        "action",
        "TNT owned by " .. owner .. " detonated at " .. core.pos_to_string(pos) ..
                " with radius " .. radius
    )
...
function unilib.pkg.shared_tnt.tnt_burn(pos, nodename)
    ...
    elseif core.get_item_group(name, "tnt") > 0 then
        ...
        core.sound_play("unilib_tnt_ignite", {pos = pos, gain = 1.0}, true)
...
function unilib.pkg.shared_tnt.tnt_boom(pos, def_table)
    ...
    if not def_table.explode_center and def_table.ignore_protection ~= true then
        core.set_node(pos, {name = "unilib:tnt_ordinary_placeholder"})
    end
    ...
    unilib.utils.log(
        "action",
        "A TNT explosion occurred at " .. core.pos_to_string(pos) .. " with radius " .. radius
    )
...
function unilib.pkg.shared_tnt.register_tnt(data_table)
        ...
        unilib.register_node(full_name, orig_name, replace_mode, {
            ...
            on_punch = function(pos, node, puncher)
                ...
                if puncher:get_wielded_item():get_name() == "unilib:torch_ordinary" then
                    ...
                    core.swap_node(pos, {name = full_name .. "_burning"})
                    core.registered_nodes[full_name .. "_burning"].on_construct(pos)
                    --[[
                    unilib.utils.log(
                        "action",
                    ...
                    unilib.utils.log_player_action(puncher, "ignites", node.name, "at", pos)
    ...
    unilib.register_node(full_name .. "_burning", nil, replace_mode, {
        ...
        on_construct = function(pos)
            ...
            core.sound_play("unilib_tnt_ignite", {pos = pos}, true)
