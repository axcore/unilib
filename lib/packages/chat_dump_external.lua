...
local function do_dump_mods(pname, param)
    ...
    -- Show details about external mods (from the file ../csv/mods/external.csv)
...
function unilib.pkg.chat_dump_external.init()

    return {
        description = "Chat command /dump_external",
        notes = "Dumps details about all external mods, or just the specified external mod",
    }

end

function unilib.pkg.chat_dump_external.exec()

    core.register_chatcommand("dump_external", {
        params = "[mod]",
        description = S(
