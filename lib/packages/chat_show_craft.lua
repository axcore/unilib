...
function unilib.pkg.chat_show_craft.exec()
    ...
    core.register_chatcommand("show_craft", {
        ...
        func = function(pname, param)
            ...
            local craft_table =
                    unilib.pkg.shared_chat_statistics.get_activity_stat(pname, "craft_table")
            if type(craft_table) ~= "table" or unilib.utils.is_table_empty(craft_table) then
                return false, S("Player @1 has never crafted anything!", pname)
            end

            unilib.utils.chat_send_player(pname, S("Player crafting list for @1", pname))
            ...
            for _, full_name in ipairs(unilib.utils.sort_table(craft_table)) do
                ...
                unilib.utils.chat_send_player(
                    pname,
                    "   " .. string.format("%-5d", craft_table[full_name]) .. " " .. full_name
                )
