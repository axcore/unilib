...
function unilib.pkg.produce_sisal.init()
...
function unilib.pkg.produce_sisal.exec()

    local orig_name_list = {}
    for i = 1, 3 do
        table.insert(orig_name_list, "better_farming:sisal_" .. i)
    end

    unilib.register_produce_fredo({
        -- From better_farming:sisal (creates unilib:produce_sisal_harvest), better_farming:sisal_1
        --      etc (creates unilib:produce_sisal_grow_1 etc)
        part_name = "sisal",
        grow_orig_name = orig_name_list,
        harvest_orig_name = "better_farming:sisal",
        description = S("Sisal"),
