...
unilib.pkg.abm_ethereal_papyrus_bamboo_spread = {}
...
local function grow_papyrus(pos, node)
    ...
    if not nod or
            minetest.get_item_group(nod.name, "soil") < 1 or
            minetest.find_node_near(pos, 3, {"group:water"}) == nil then
        return
    end
...
function unilib.pkg.abm_ethereal_papyrus_bamboo_spread.init()

    return {
        description = "Replacment ABM to handle spread of ordinary papyrus and bamboo",
        at_least_one = {"plant_papyrus_ordinary", "tree_bamboo"},
    }

end

function unilib.pkg.abm_ethereal_papyrus_bamboo_spread.post()
    ...
    for _, abm_def in pairs(minetest.registered_abms) do

        local label = abm_def.label or ""
        local node = abm_def.nodenames and abm_def.nodenames[1] or ""

        if label == "Ordinary papyrus growth" or node == "unilib:plant_papyrus_ordinary" then
