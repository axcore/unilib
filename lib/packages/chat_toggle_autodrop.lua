...
local function do_toggle(pname, param)
    ...
    if not unilib.setting.magnet_auto_drop_flag then
        ...
        local enabled_flag = unilib.magnet.toggle_player_auto_drop(pname)
        if not enabled_flag then
            return true, S("Auto-drop has been disabled for @1", pname)
        else
            return true, S("Auto-drop has been enabled for @1", pname)
        end
