...
unilib.pkg.mapgen_underch_jit_unilib = {}
...
--      ore
-- Note that these statistics are not very accurate; they don't take account of ores places via
...
-- Table identical to the one used in ../shared/stone.lua, converting a PFAA hardness (in the range
--      1-5) into a Minetest cracky group
local smooth_cracky_table = {3, 2, 1, 1, 1}

-- When placing ore distributions in place_blob() and place_scatter(), introduce randomness
-- (Use our own values for our own functions, rather than using the ../shared/jbiomes.lua
--      equivalents)
...
--      the upper limit specified by unilib.underch_max_biomes (currently 62)
...
-- Before setting any of the flags below to true, you should edit the "underch_jit_unilib" remix to
--      uncomment the necessary packages (i.e. by removing the # character at the beginning of each
--      line)
...
local blob_default_flag = true
local scatter_default_flag = true
local scatter_moreores_flag = true
local scatter_technic_flag = true
local scatter_xtraores_flag = true
...
local debug_enable_single_flag = false
local debug_enable_blob_flag = false
local debug_enable_scatter_flag = false
...
local function balance_ores(basic_stone_count)
    ...
    if not ore_balance_flag then
        ...
        for item_name, chance in pairs (ore_chance_table) do
            ore_factor_table[item_name] = unilib.round_up((average_chance / chance), 2)
        end
...
local function illuminate_nodes()
    ...
    if debug_glow_flag then
        ...
        unilib.override_item("unilib:misc_matrix_bulk", {
            light_source = 7,
        })

        unilib.override_item("unilib:misc_matrix_crust", {
            light_source = 7,
        })
...
local function prepare_height_adjust()
    ...
    elseif ore_height_adjust_below_flag then
        ...
        for i = -1, -310, -1 do
            ...
            height_adjust_table[i] = unilib.round_up(
        ...
        for i = 0, 621 do
            ...
            height_adjust_table[310 - i] = unilib.round_up(
...
local function add_ore(data_table)
    ...
    local img = minetest.registered_nodes["unilib:" .. wherein].tiles[1]
    local inv_hide = nil
    ...
    if metal ~= nil then
        ...
        full_name = "unilib:" .. wherein .. "_with_" .. metal
        -- (Checking that the lump craftitem exists is more reliable than checking
        --      unilib.metal_table, at least at the time of writing)
        if lump_texture_flag == true and
                minetest.registered_craftitems["unilib:metal_" .. metal .. "_lump"] ~= nil then
    ...
    elseif mineral ~= nil then
        ...
        full_name = "unilib:" .. wherein .. "_with_" .. mineral
        -- (Checking that the lump craftitem exists is more reliable than checking
        --      unilib.mineral_table, at least at the time of writing)
        if lump_texture_flag == true and
                minetest.registered_craftitems["unilib:mineral_" .. mineral .. "_lump"] ~= nil then
        ...
        unilib.show_warning("mapgen_underch_jit_unilib package, add_ore(): Invalid arguments")
        return

    end

    -- Don't create the same new ore twice, e.g. e.g. the skarn ores created by the rocks mod will
    --      have the same name as some nodes created by this package
    if minetest.registered_nodes[full_name] ~= nil then

        if debug_duplicate_ore_flag then

            unilib.show_warning(
                "mapgen_underch_jit_unilib package, add_ore(): Ore already exists", full_name
            )
...
local function distribute_single_ore(data_table)
    ...
    local y_max = data_table.y_max or unilib.y_max
    local y_min = data_table.y_min or unilib.y_min

    -- (Checks are required, because non-existent nodes can cause a crash)
    if (
        wherein ~= "all" and
        minetest.registered_nodes["unilib:" .. wherein] == nil
    ) or (
        minetest.registered_nodes["unilib:" .. ore] == nil
    ) then

        unilib.show_warning(
            "mapgen_underch_jit_unilib package, distribute_single_ore(): Unrecognised node(s)",
            "unilib:" .. wherein,
            "unilib:" .. ore
        )
    ...
    if alt_ore ~= nil then
        ...
        if minetest.registered_nodes["unilib:" .. alt_ore] == nil then
...
local function distribute_blob(data_table)
    ...
    if (
        ...
        unilib.show_warning(
            "mapgen_underch_jit_unilib package, distribute_blob(): Unrecognised node(s)",
            "unilib:" .. wherein,
            "unilib:" .. ore
        )
...
local function distribute_scatter(data_table)
    ...
    if (
        ...
        unilib.show_warning(
            "mapgen_underch_jit_unilib package, distribute_scatter(): Unrecognised node(s)",
            "unilib:" .. wherein,
            "unilib:" .. ore
        )
...
local function place_single_ore(pos, distrib_table, item_name, height_adjust)

    -- Adapted from unilib.jbiome_ore() in ../shared/jbiomes.lua
    -- That function uses the arguments (pos, node1, node2, chance), but here we assume that values
    --      in "distrib_table" are correct

    local full_name = "unilib:" .. distrib_table.ore

    if debug_ore_flag and minetest.registered_nodes[full_name] == nil then

        unilib.show_warning(
            "mapgen_underch_jit_unilib package, place_single_ore(): Undefined ore", full_name
        )
    ...
    if distrib_table.y_min <= pos.y and
            distrib_table.y_max >= pos.y and
            math.random() < (
                distrib_table.chance * ore_factor_table[item_name] * height_adjust
            ) then
...
local function place_blob(pos, distrib_table)

    -- Adapted from unilib.jbiome_blob() in ../shared/jbiomes.lua
    ...
    if debug_ore_flag and minetest.registered_nodes[full_name] == nil then
        ...
        unilib.show_warning(
            "mapgen_underch_jit_unilib package, place_blob(): Undefined blob", full_name
        )
    ...
    if (pos.y < distrib_table.y_min) or (pos.y > distrib_table.y_max) then
        return false
    end

    local xx = math.abs(pos.x + x_off * distrib_table.period3)
    local yy = math.abs(pos.y + y_off * distrib_table.period)
    local zz = math.abs(pos.z + z_off * distrib_table.period2)
    ...
    if t1 < distrib_table.size then
        ...
        minetest.set_node(pos, {name = "unilib:" .. distrib_table.ore})
        return true
...
local function place_scatter(pos, distrib_table)

    -- Adapted from unilib.jbiome_scatter() in ../shared/jbiomes.lua
    -- As described above, here we assume that the values in "distrib_table" are correct
    ...
    if debug_ore_flag and minetest.registered_nodes[full_name] == nil then
        ...
        unilib.show_warning(
            "mapgen_underch_jit_unilib package, place_scatter(): Undefined scatter", full_name
        )
    ...
    local xx = math.abs(pos.x + x_off * distrib_table.period2)
    local yy = math.abs(pos.y + y_off * distrib_table.period3)
    local zz = math.abs(pos.z + z_off * distrib_table.period)
...
local function populate_ore_tables()
    ...
    --      unilib.pkg.mapgen_underch_jit_unilib.exec()
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Alunite Ore"), S("Pale Andesite")),
        level = 2,
    })
    ...
    for _, part_name in pairs(basalt_list) do
        ...
        local data_table = unilib.stone_table[part_name]
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Cinnabar Ore"), wherein_description),
            level = 1,
        })
        distribute_single_ore({
            wherein = item_name,
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Chromite Ore"), S("Dark Diorite")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_diorite_dark",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Perlite Ore"), S("Dark Diorite")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_diorite_dark",
    ...
    for _, part_name in pairs(gabbro_list) do
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Perlite Ore"), wherein_description),
            level = 3,
        })
        distribute_single_ore({
            wherein = item_name,
    ...
    for _, part_name in pairs(granite_list) do
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Cassiterite Ore"), wherein_description),
            level = 3,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        if add_native_flag then
            ...
            add_ore({
                ...
                description = unilib.brackets(S("Native Tin Ore"), wherein_description),
                level = 1,
            })
            distribute_single_ore({
                wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Native Gold Ore"), wherein_description),
            level = 3,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        if add_native_flag then
            ...
            add_ore({
                ...
                description = unilib.brackets(S("Native Silver Ore"), wherein_description),
                level = 3,
            })
            distribute_single_ore({
                wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Scheelite Ore"), wherein_description),
            level = 2,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Chalcopyrite Ore"), wherein_description),
            level = 2,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Magnetite Ore"), wherein_description),
            level = 3,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Stibnite Ore"), wherein_description),
            level = 1,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Brown Quartz Ore"), wherein_description),
            level = 3,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Realgar Ore"), wherein_description),
            level = 1,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Fluorite Ore"), wherein_description),
            level = 1,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        if add_native_flag then
            ...
            add_ore({
                ...
                description = unilib.brackets(S("Native Bismuth Ore"), wherein_description),
                level = 1,
            })
            distribute_single_ore({
                wherein = item_name,
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Perlite Ore"), S("Pale Peridotite")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_peridotite_pale",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Alunite Ore"), S("Pale Rhyolite")),
        level = 2,
    })
    distribute_single_ore({
        wherein = "stone_rhyolite_pale",
    ...
    for part_name, data_table in pairs(unilib.stone_table) do
        ...
        if not data_table.no_smooth_flag and data_table.category == "sedimentary" then
            ...
            add_ore({
                ...
                description = unilib.brackets(S("Anthracite Ore"), wherein_description),
                level = 3,
            })
            distribute_single_ore({
                wherein = item_name,
            ...
            add_ore({
                ...
                description = unilib.brackets(S("Bituminous Coal Ore"), wherein_description),
                level = 1,
            })
            distribute_single_ore({
                wherein = item_name,
            ...
            add_ore({
                ...
                description = unilib.brackets(S("Lignite Ore"), wherein_description),
                level = 1,
            })
            distribute_single_ore({
                wherein = item_name,
            ...
            add_ore({
                ...
                description = unilib.brackets(S("Natural Coke Ore"), wherein_description),
                level = 3,
            })
            distribute_single_ore({
                wherein = item_name,
            ...
            add_ore({
                ...
                description = unilib.brackets(S("Pyrolusite Ore"), wherein_description),
                level = 1,
            })
            distribute_single_ore({
                wherein = item_name,
            ...
            add_ore({
                ...
                description = unilib.brackets(S("Apatite Ore"), wherein_description),
                level = 3,
            })
            distribute_single_ore({
                wherein = item_name,
            ...
            if add_native_flag then
                ...
                add_ore({
                    ...
                    description = unilib.brackets(S("Native Platinum Ore"), wherein_description),
                    level = 3,
                })
                distribute_single_ore({
                    wherein = item_name,
    ...
    for _, part_name in pairs(serpentinite_list) do
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Steatite Ore"), wherein_description),
            level = 1,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Magnesite Ore"), wherein_description),
            level = 2,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Chrysotile Ore"), wherein_description),
            level = 1,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Pentlandite Ore"), wherein_description),
            level = 2,
        })
        distribute_single_ore({
            wherein = item_name,
    ...
    for _, part_name in pairs(limestone_list) do
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Gypsum Ore"), wherein_description),
            level = 1,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Halite Ore"), wherein_description),
            level = 1,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Celestine Ore"), wherein_description),
            level = 3,
        })
        distribute_single_ore({
            wherein = item_name,
    ...
    for _, part_name in pairs(sandstone_list) do
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Trona Ore"), wherein_description),
            level = 1,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Mirabilite Ore"), wherein_description),
            level = 1,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Borax Ore"), wherein_description),
            level = 1,
        })
        distribute_single_ore({
            wherein = item_name,
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Spodumene Ore"), S("Pale Pegmatite")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_pegmatite_white",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Mica Ore"), S("Pale Pegmatite")),
        level = 1,
    })
    distribute_single_ore({
        wherein = "stone_pegmatite_white",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Kyanite Ore"), S("Pale Pegmatite")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_pegmatite_white",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Lepidolite Ore"), S("Pale Pegmatite")),
        level = 2,
    })
    distribute_single_ore({
        wherein = "stone_pegmatite_white",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Tantalite Ore"), S("Pale Pegmatite")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_pegmatite_white",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Cassiterite Ore"), S("Pale Pegmatite")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_pegmatite_white",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Wolframite Ore"), S("Pale Pegmatite")),
        level = 2,
    })
    distribute_single_ore({
        wherein = "stone_pegmatite_white",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Pollucite Ore"), S("Pale Pegmatite")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_pegmatite_white",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Bastnasite Ore"), S("Carbonatite")),
        level = 2,
    })
    distribute_single_ore({
        wherein = "stone_carbonatite",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Apatite Ore"), S("Carbonatite")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_carbonatite",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Chalcopyrite Ore"), S("Carbonatite")),
        level = 2,
    })
    distribute_single_ore({
        wherein = "stone_carbonatite",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Barite Ore"), S("Carbonatite")),
        level = 2,
    })
    distribute_single_ore({
        wherein = "stone_carbonatite",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Uraninite Ore"), S("Carbonatite")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_carbonatite",
    ...
    for _, part_name in pairs(sandstone_list) do
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Uraninite Ore"), wherein_description),
            level = 3,
        })
        distribute_single_ore({
            wherein = item_name,
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Uraninite Ore"), S("Rough Conglomerate")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_conglomerate_rough",
    ...
    for part_name, data_table in pairs(unilib.stone_table) do
        ...
        if not data_table.no_smooth_flag and
            ...
            add_ore({
                ...
                description = unilib.brackets(S("Sphalerite Ore"), wherein_description),
                level = 2,
            })
            distribute_single_ore({
                wherein = item_name,
            ...
            add_ore({
                ...
                description = unilib.brackets(S("Galena Ore"), wherein_description),
                level = 1,
            })
            distribute_single_ore({
                wherein = item_name,
            ...
            add_ore({
                ...
                description = unilib.brackets(S("Barite Ore"), wherein_description),
                level = 2,
            })
            distribute_single_ore({
                wherein = item_name,
    ...
    for _, part_name in pairs(skarn_list) do
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Malachite Ore"), wherein_description),
            level = 2,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Wollastonite Ore"), wherein_description),
            level = 2,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Lapis Lazuli Ore"), wherein_description),
            level = 2,
        })
        distribute_single_ore({
            wherein = item_name,
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Dolomite Ore"), ("Dark Dolostone")),
        level = 1,
    })
    distribute_single_ore({
        wherein = "stone_dolostone_dark",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Magnesite Ore"), ("Dark Dolostone")),
        level = 2,
    })
    distribute_single_ore({
        wherein = "stone_dolostone_dark",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Steatite Ore"), ("Dark Dolostone")),
        level = 1,
    })
    distribute_single_ore({
        wherein = "stone_dolostone_dark",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Wollastonite Ore"), S("Dark Dolostone")),
        level = 2,
    })
    distribute_single_ore({
        wherein = "stone_dolostone_dark",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Magnesite Ore"), ("Pale Peridotite")),
        level = 2,
    })
    distribute_single_ore({
        wherein = "stone_peridotite_pale",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Vanadium Magnetite Ore"), ("Pale Peridotite")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_peridotite_pale",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Pentlandite Ore"), ("Pale Peridotite")),
        level = 2,
    })
    distribute_single_ore({
        wherein = "stone_peridotite_pale",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Serpentinite Ore"), ("Pale Peridotite")),
        level = 1,
    })
    distribute_single_ore({
        wherein = "stone_peridotite_pale",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Ilmenite Ore"), ("Dark Diorite")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_diorite_dark",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Chromite Ore"), ("Dark Diorite")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_diorite_dark",
    ...
    for _, part_name in pairs(gabbro_list) do
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Vanadium Magnetite Ore"), wherein_description),
            level = 3,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Serpentinite Ore"), wherein_description),
            level = 2,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Ilmenite Ore"), wherein_description),
            level = 3,
        })
        distribute_single_ore({
            wherein = item_name,
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Chromite Ore"), wherein_description),
            level = 3,
        })
        distribute_single_ore({
            wherein = item_name,
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Chalcopyrite Ore"), ("Pale Rhyolite")),
        level = 2,
    })
    distribute_single_ore({
        wherein = "stone_rhyolite_pale",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Sphalerite Ore"), ("Pale Rhyolite")),
        level = 2,
    })
    distribute_single_ore({
        wherein = "stone_rhyolite_pale",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Galena Ore"), ("Pale Rhyolite")),
        level = 1,
    })
    distribute_single_ore({
        wherein = "stone_rhyolite_pale",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Native Gold Ore"), ("Pale Rhyolite")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_rhyolite_pale",
    ...
    if add_native_flag then
        ...
        add_ore({
            ...
            description = unilib.brackets(S("Native Silver Ore"), ("Pale Rhyolite")),
            level = 3,
        })
        distribute_single_ore({
            wherein = "stone_rhyolite_pale",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Pyrite Ore"), ("Pale Rhyolite")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_rhyolite_pale",
    ...
    for part_name, data_table in pairs(unilib.stone_table) do
        ...
        if not data_table.no_smooth_flag and data_table.category == "sedimentary" then
            ...
            add_ore({
                ...
                description = unilib.brackets(S("Haematite Ore"), wherein_description),
                level = 3,
            })
            distribute_single_ore({
                wherein = item_name,
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Chalcopyrite Ore"), ("Breccia")),
        level = 2,
    })
    distribute_single_ore({
        wherein = "stone_diorite_dark",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Molybdenite Ore"), ("Breccia")),
        level = 1,
    })
    distribute_single_ore({
        wherein = "stone_diorite_dark",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Bauxite Ore"), ("Laterite")),
        level = 1,
    })
    distribute_single_ore({
        wherein = "stone_laterite",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Brown Limonite Ore"), ("Laterite")),
        level = 1,
    })
    distribute_single_ore({
        wherein = "stone_laterite",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Yellow Limonite Ore"), ("Laterite")),
        level = 1,
    })
    distribute_single_ore({
        wherein = "stone_laterite",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Garnierite Ore"), ("Dark Serpentinite")),
        level = 2,
    })
    distribute_single_ore({
        wherein = "stone_serpentinite_dark",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Kimberlite Ore"), S("Serpentinite")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_serpentinite_dark",
    ...
    add_ore({
        ...
        description = unilib.brackets(S("Diamond Ore"), ("Serpentinite")),
        level = 3,
    })
    distribute_single_ore({
        wherein = "stone_serpentinite_dark",
    ...
    for part_name, data_table in pairs(unilib.stone_table) do
        ...
        if not data_table.no_smooth_flag and data_table.category == "metamorphic" then
            ...
            add_ore({
                ...
                description = unilib.brackets(S("Cuprite Ore"), wherein_description),
                level = 2,
            })
            distribute_single_ore({
                wherein = item_name,
            ...
            add_ore({
                ...
                description = unilib.brackets(S("Mese Ore"), wherein_description),
                level = 3,
            })
            distribute_single_ore({
                wherein = item_name,
            ...
            if add_native_flag then
                ...
                add_ore({
                    ...
                    description = unilib.brackets(S("Native Copper Ore"), wherein_description),
                    level = 3,
                })
                distribute_single_ore({
                    wherein = item_name,
    ...
    if add_real_minerals_flag then
        ...
        local real_minerals_list = {
            ...
            {"sulphur_real",    "mineral_sulphur_real_rock",    0.1,          nil},
            {"sylvite",         "mineral_sylvite_rock",         0.1,          nil}, -- potassium
            {"tenorite",        "mineral_tenorite_rock",        0.1,          nil},
            {"tetrahedrite",    "mineral_tetrahedrite_rock",    0.1,          nil}, -- real copper
...
--          {"bismuthinite",    "mineral_bismuthinite_rock",    0.1,          nil},
    ...
    for _, mini_list in ipairs(additional_list) do
        ...
        local stone_data_table = unilib.stone_table[wherein]

        if ore ~= nil then

            local mineral_data_table = unilib.mineral_table[ore]
...
local function add_jit_property(full_name)
    ...
    -- Adapted from the same function in the "mapgen_underch_jit" package

    -- (The if-condition is simpler than checking the parent package has been executed. It also
    --      catches any stones which don't have a mossy cobble variant)
    if minetest.registered_nodes[full_name] ~= nil then
...
local function override_nodes()
    ...
    -- Adapted from the same function in the "mapgen_underch_jit" package
    -- Overrides various nodes to make them compatible with JIT. Note that new ores created above
    --      by a call to add_ore() are already compatible with JIT

    -- Various things from default. This is a more complete list than the equivalent one in the
    --      "mapgen_underch_jit" package. However, note that some of the nodes created by these
    --      packages are not used by populate_ore_tables()
    for _, item_name in ipairs({
        ...
        add_jit_property("unilib:" .. item_name)
    end

    -- Clay/dirt/sand packages, all of which are used by populate_ore_tables()
    for _, item_name in ipairs({
        "clay_attapulgite",
        "clay_bentonite_calcium",
        "clay_bentonite_sodium",
        "clay_kaolinite",
        "clay_vermiculite",
    ...
    -- Stone packages, most of which are not used by populate_ore_tables()
    for _, part_name in ipairs(basic_stone_list) do

        add_jit_property("unilib:stone_" .. part_name)
        if minetest.registered_nodes["unilib:stone_" .. part_name .. "_cobble"] ~= nil then

            add_jit_property("unilib:stone_" .. part_name .. "_cobble")
            add_jit_property("unilib:stone_" .. part_name .. "_cobble_mossy")

        elseif minetest.registered_nodes["unilib:stone_" .. part_name .. "_rubble"] ~= nil then

            add_jit_property("unilib:stone_" .. part_name .. "_rubble")
...
local function populate_id_table()
    ...
    for _, ending in ipairs(ending_list) do
        ...
        local full_name = "unilib:stone_ordinary_" .. ending

        if minetest.registered_nodes[full_name] ~= nil then
            id_table["stone_ordinary_" .. ending] = minetest.get_content_id(full_name)
        end
...
local function setup_lbm()
    ...
    minetest.register_lbm{
        label = "Replace crust with bulk [mapgen_underch_jit_unilib]",
        name = "unilib:lbm_mapgen_underch_jit_unilib",
...
local function switch_stone(pos, item_name)

    -- Adapted from underch/jit.lua, underch.jit.use_stone()

    local height_adjust = height_adjust_table[math.floor(pos.y / 100)]
    ...
    if name == "unilib:misc_matrix_bulk" or name == "unilib:misc_matrix_crust" then
        ...
        if debug_enable_single_flag and single_ore_table[item_name] ~= nil then

            for _, distrib_table in ipairs(single_ore_table[item_name]) do

                -- (As soon as the matrix node is switched for an ore, we can stop checking
                if place_single_ore(pos, distrib_table, item_name, height_adjust) then
                    return
                end
        ...
        if debug_enable_blob_flag and blob_table[item_name] ~= nil then

            for _, distrib_table in ipairs(blob_table[item_name]) do

                if place_blob(pos, distrib_table) then
                    return
                end
        ...
        if debug_enable_scatter_flag and scatter_table[item_name] ~= nil then

            for _, distrib_table in ipairs(scatter_table[item_name]) do

                if place_scatter(pos, distrib_table) then
                    return
                end
        ...
        if debug_enable_single_flag and single_ore_table["all"] ~= nil then
        ...
        if debug_enable_blob_flag and blob_table["all"] ~= nil then
        ...
        if debug_enable_scatter_flag and scatter_table["all"] ~= nil then
        ...
        minetest.set_node(pos, {name = "unilib:" .. item_name})
...
function unilib.pkg.mapgen_underch_jit_unilib.init()

    return {
        description = "Pseudo-biomes for the \"underch_jit_unilib\" remix",
        notes = "A simplistic implementation of PFAA Geologica's geology system, partly based" ..
                ...
                " This package assumes that all packages listed in the \"underch_jit_unilib\"" ..
                " remix have been executed (rather than listing them all here as dependencies)",
        excludes = {"mapgen_underch", "mapgen_underch_jit"},
        depends = "stone_ordinary",
    }

end

function unilib.pkg.mapgen_underch_jit_unilib.post()
    ...
    for i = 1, basic_stone_count do
        matrix_table[i] = function(x, y, z, vi, data, p2data, area, lastlayer) end
    end
    ...
    if force_biome == nil then
        ...
        for i = 1, basic_stone_count do
            ...
            converted_table[i] = function(pos)
                switch_stone(pos, "stone_" .. basic_stone_list[i])
            end
        ...
        -- Use only one pseudo-biome (for testing purposes)
        for i = 1, basic_stone_count do

            converted_table[i] = function(pos)
                switch_stone(pos, "stone_" .. basic_stone_list[force_biome])
            end
    ...
    -- There are also two ore-generation tables, independent of those used above, used to place
    --      common nodes, and common ores in ordinary stone. This ensures that items that are
    --      important for crafting can be commonly available to players, even though they might be
    --      rare in the ores generated by populate_ore_tables()
    ...
    if scatter_xtraores_flag then
        ...
        table.insert(
            scatter_table,
            {unilib.y_min, -2000, nil, "unilib:stone_ordinary_with_ozymandium", 195, 28, 30, 32}
        )
        table.insert(
            scatter_table,
            {unilib.y_min, -5000, nil, "unilib:stone_ordinary_with_adamantite", 200, 31, 32, 33}
        )
        table.insert(
            scatter_table,
            {unilib.y_min, -10000, nil, "unilib:stone_ordinary_with_rarium", 205, 30, 34, 37}
        )
        table.insert(
            scatter_table,
            {unilib.y_min, -15000, nil, "unilib:stone_ordinary_with_unobtainium", 210, 29, 37, 34}
        )
        table.insert(
            scatter_table,
            {unilib.y_min, -20000, nil, "unilib:stone_ordinary_with_tritonium", 215, 32, 29, 37}
        )
        table.insert(
            scatter_table,
            {unilib.y_min, -25000, nil, "unilib:stone_ordinary_with_geminitinum", 220, 32, 33, 35}
        )
    ...
    -- Pass this data to the code in ../lib/shared/jbiomes.lua
    ...
    if debug_no_ore_flag then
        ...
        for i = 1, basic_stone_count do
            ...
            if single_ore_table["stone_" .. part_name] == nil and
                ...
                unilib.show_msg(
                    "mapgen_underch_jit_unilib package: basic stone has no ores", part_name
                )
